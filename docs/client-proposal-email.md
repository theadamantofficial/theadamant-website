...
opens the proposal composer. Corporate
...
the recipient, subject and message before sending.
...
Deploy the website changes. Proposal emails contain the reviewed message and
links; no file attachment is uploaded or sent.

Open a lead's **Send proposal via CRM** action, select its recipient address, review the content,
then choose **Send proposal**. Sending is disabled until the proposal
