...
export default function ProcessSection({copy}: {copy: SiteCopy["process"]}) {
    ...
    return <section id="process" className="section-shell py-24" aria-labelledby="process-heading">
        <SectionDepth variant="process"/>
        <MascotHeading mood="walking-tablet" side="right">
            <Reveal className="max-w-3xl">
                <p className="section-kicker">{copy.kicker}</p>
                <h2 id="process-heading" className="section-title">{copy.title}</h2>
                <p className="section-copy">{copy.description}</p>
            </Reveal>
        </MascotHeading>
        <div className="mt-10 h-1 overflow-hidden rounded-full bg-foreground/8" aria-hidden="true">
            <div className="process-progress h-full origin-left rounded-full bg-gradient-to-r from-primary to-accent"/>
        </div>
        ...
        <StaggerGroup className="mt-5 grid gap-4 lg:auto-rows-fr lg:grid-cols-4">
            {copy.steps.map(({number, title, description}, index) => {
                ...
                return <StaggerItem key={number} className="h-full">
                    <article className="glass-panel lift-card flex h-full flex-col p-6">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-foreground text-background"><Icon className="h-5 w-5"/></div>
                        </div>
