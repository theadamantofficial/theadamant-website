...
export function PipelineSwitcher({value}: {value: "sales" | "development"}) {
    ...
    return <div className="mb-6 flex flex-wrap items-center justify-between gap-3 border-b border-[var(--crm-border)] pb-4">
        <label className="flex items-center gap-2 text-sm font-semibold"><Workflow className="h-4 w-4 text-violet-500"/><span className="sr-only">Pipeline workspace</span><select aria-label="Pipeline workspace" value={value} onChange={(event) => router.push(event.target.value === "sales" ? "/admin/pipeline" : "/admin/development")} className="crm-control min-w-52">
