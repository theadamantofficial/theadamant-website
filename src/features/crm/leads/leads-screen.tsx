...
export function LeadsScreen({initialSearch = ""}: {initialSearch?: string}) {
    ...
    useEffect(() => { void crmFetch<{members: TeamMember[]}>("/api/admin/team").then((data) => setMembers(data.members)).catch(() => undefined); }, []);
    ...
    return <div className="space-y-5">
        <PageHeader title="Leads" description="Manage prospects and opportunities." actions={<Link href="/admin/leads/new" className="crm-button-primary"><Plus className="h-3.5 w-3.5"/> Add lead</Link>}/>
        ...
        <section className="crm-card overflow-hidden">
            <div className="border-b border-[var(--crm-border)] p-3 sm:p-4">
                <div className="grid gap-2 md:grid-cols-[minmax(12rem,1fr)_repeat(4,minmax(8rem,auto))_auto]">
                    ...
                    <FilterSelect label="Assigned user" value={assignedTo} onChange={setAssignedTo}><option value="all">All owners</option><option value="unassigned">Unassigned</option>{members.filter((member) => member.active).map((member) => <option key={member.id} value={member.id}>{member.full_name || member.email}</option>)}</FilterSelect>
