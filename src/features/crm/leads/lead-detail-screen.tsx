...
import {CalendarClock, Check, Edit3, ExternalLink, Loader2, MessageCircle, MoreHorizontal, Phone, Plus, Send, UserRound} from "lucide-react";
import {DataError, EmptyState, Modal, PriorityBadge, Skeleton, StatusBadge, UserAvatar} from "@/components/admin/admin-ui";
import {LEAD_SOURCE_LABELS, LEAD_STATUS_LABELS} from "@/features/crm/constants";
...
export function LeadDetailScreen({leadId}: {leadId: string}) {
    ...
    async function addNote(event: FormEvent<HTMLFormElement>) {
        ...
        try { await crmFetch(`/api/admin/leads/${leadId}/notes`, {method: "POST", body: JSON.stringify({note})}); form.reset(); toast.success("Note added"); await load(); setTab("notes"); }
        catch (noteError) { toast.error(noteError instanceof Error ? noteError.message : "Note could not be added."); }
    ...
    return <div className="space-y-5">
        ...
        {taskOpen ? <Modal title="Create task" description={`Add a task for ${lead.customer_name}.`} onClose={() => setTaskOpen(false)}><TaskForm leadId={lead.id} onCancel={() => setTaskOpen(false)} onCreated={() => { setTaskOpen(false); toast.success("Task created"); void load(); }}/></Modal> : null}
    </div>;
}
...
function NotesPanel({notes, onSubmit, saving}: {notes: LeadNote[]; onSubmit: (event: FormEvent<HTMLFormElement>) => void; saving: boolean}) {
    return <div><form onSubmit={onSubmit} className="border-b border-[var(--crm-border)] p-5"><textarea name="note" required rows={3} className="admin-input h-auto resize-y py-3" placeholder="Add a note or conversation summary…"/><div className="mt-2 flex justify-end"><button disabled={saving} className="crm-button-primary">{saving ? <Loader2 className="h-3.5 w-3.5 animate-spin"/> : <Send className="h-3.5 w-3.5"/>} Add note</button></div></form>{notes.length ? <div className="divide-y divide-[var(--crm-border)]">{notes.map((note) => <div key={note.id} className="flex gap-3 p-5"><UserAvatar size="sm" name={note.author?.full_name || note.author?.email || "Team"}/><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-3"><p className="text-xs font-semibold">{note.author?.full_name || note.author?.email || "Team member"}</p><time className="text-[10px] text-[var(--crm-muted)]">{formatCrmDate(note.created_at, true)}</time></div><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-[var(--crm-muted)]">{note.note}</p></div></div>)}</div> : <EmptyState title="No notes yet" description="Add context from calls, meetings or discovery conversations."/>}</div>;
}

function TasksPanel({tasks, onAdd, onComplete}: {tasks: CrmTask[]; onAdd: () => void; onComplete: (task: CrmTask) => Promise<void>}) {
    return <div><div className="flex justify-end border-b border-[var(--crm-border)] p-3"><button onClick={onAdd} className="crm-button-secondary"><Plus className="h-3.5 w-3.5"/> Add task</button></div>{tasks.length ? <div className="divide-y divide-[var(--crm-border)]">{tasks.map((task) => <div key={task.id} className="flex items-start gap-3 p-4"><button disabled={task.status === "completed"} onClick={() => void onComplete(task)} className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border ${task.status === "completed" ? "border-emerald-500 bg-emerald-500 text-white" : "border-[var(--crm-border)] hover:border-[#0d5c63]"}`}>{task.status === "completed" ? <Check className="h-3 w-3"/> : null}</button><div className="min-w-0 flex-1"><p className={`text-xs font-semibold ${task.status === "completed" ? "text-[var(--crm-muted)] line-through" : ""}`}>{task.title}</p><div className="mt-1.5 flex flex-wrap items-center gap-2 text-[10px] text-[var(--crm-muted)]"><CalendarClock className="h-3 w-3"/>{formatRelativeDate(task.due_date)}<span>·</span><PriorityBadge priority={task.priority}/></div></div></div>)}</div> : <EmptyState title="No tasks for this lead" description="Create a task to keep the next action visible."/>}</div>;
}
